...
# Commented out datasets not included in this clean version
...
from graphgps.loader.split_generator import (prepare_splits,
                                             set_dataset_splits)
# Simplified stub functions for features not used in Sioux Falls traffic task
...
def task_specific_preprocessing(data, cfg):
    """Simplified stub: Sioux Falls dataset doesn't need special preprocessing."""
    return data


def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    """Simplified stub: Positional encodings disabled in Sioux Falls config."""
    return data
...
def log_loaded_dataset(dataset, format, name):
    ...
    try:
        is_undir = dataset[0].is_undirected()
    except AttributeError:
        is_undir = "Custom (edge_index_old & new)"
    logging.info(f"  undirected: {is_undir}")
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        elif pyg_dataset_id == 'SiouxFalls':
...
def preformat_SiouxFalls(dataset_dir, name):
    """Load and preformat Sioux Falls Traffic Flow dataset.

    Args:
        dataset_dir: path where to store the cached dataset
        name: not used, kept for consistency

    Returns:
        PyG dataset object
    """
    # Use the actual data path, not the auto-generated one
    # The actual data is in create_sioux_data/processed_data
    actual_data_path = 'create_sioux_data/processed_data'
    
    dataset = join_dataset_splits(
        [SiouxFallsTrafficDataset(root=actual_data_path, split=split)
         for split in ['train', 'val', 'test']]
    )
    return dataset
...
def preformat_NetworkPairs(dataset_dir, name):
    ...
    split_probe = 'train_dataset.pt'
    candidate_dirs = [dataset_dir, osp.dirname(dataset_dir)]
    actual_dir = next(
        (path for path in candidate_dirs if osp.exists(osp.join(path, split_probe))),
        dataset_dir,
    )
    cfg.dataset.processed_root = actual_dir
    cfg.dataset.dir = actual_dir

    metadata_path = osp.join(actual_dir, 'dataset_meta.json')
    if osp.exists(metadata_path):
        ...
        for field in [
            ...
            if field in metadata:
                setattr(cfg.dataset, field, metadata[field])
        logging.info(
    ...
    try:
        ...
        raise FileNotFoundError(
            ...
            f"Configured processed dataset dir: {actual_dir}\n"
            f"Generate the processed dataset first, then retry training:\n"
            f"  1. python solve_network_pairs.py --network_name <SiouxFalls|EMA|Anaheim>\n"
            f"  2. python build_network_pairs_dataset.py --output_dir {actual_dir}\n"
        ) from None
